const d_main = 'learn.01founders.co'
const g_ep = `https://${d_main}/api/graphql-engine/v1/graphql`
...
const fetchGraphQL = async (query, variables) => {
    const response = await fetch(g_ep, {
        method: "POST",
...
const parseUserInfo = async () => {
    ...
    me.id = obj.data.user[0].id
    me.login = obj.data.user[0].login
}
console.log(fetchGraphQL + "lol")
...
const parseTransactions = async () => {
    ...
    while (true) {
        const obj = await fetchGraphQL(`
            query transactions($login: String, $offset: Int) {
                transaction(
                    where: {
        ...
        me.transactions.push(...obj.data.transaction)
...
const parseProgresses = async () => {
    ...
    while (true) {
        const obj = await fetchGraphQL(`
            query progression ($login: String, $offset: Int) {
                progress(
                    where: {
                        user: { login: { _eq: $login } }
                        isDone: { _eq: true }
        ...
        me.progression.push(...obj.data.progress)
...
const parseProjectsBaseXP = () => {
    me.transactions.forEach(transaction => {
        if (me.progression.find(progress => progress.object.id == transaction.object.id)) {
...
const parseDoneProjects = () => {
    me.transactions.forEach(transaction => {
        ...
        if (projectsBaseXP && projectBaseXP == transaction.amount) {
            me.totalXP += projectBaseXP
            const newLevel = getLevelFromXp(me.totalXP)
    ...
    me.doneProjects.sort((a, b) => a.date > b.date ? 1 : -1 )}

// total xp needed for this level
const totalXPForLevel = (level) => Math.round((level * 0.66 - 1 ) * ((level) * 150 + 50))
...
const fillGraphs = (xpOverTimeGraph, levelOverTimeGraph) => {
    const firstDate = getFirstDayOfMonth(me.doneProjects[0].date)
    const lastDate = getFirstDayOfNextMonth(me.doneProjects[me.doneProjects.length - 1].date)
    ...
    for (let i = 0; i <= 10; i++) {
        ...
        const text = (i == 10 ? 0 : Math.round(me.totalXP * (1 - (i / 10)))).toLocaleString()
        const type = 'y-label'
    ...
    for (let i = 0; i <= me.level; i++) {
        const x = levelOverTimeGraph.leftOffset * 0.8
        const y = (i == 0 ? levelOverTimeGraph.height : (levelOverTimeGraph.height * (1 - (i / me.level)))) + 5
        const text = i
        const type = 'y-label'
    ...
    for (let i = 1; i < me.doneProjects.length; i++) {
        const curr = me.doneProjects[i]
        const prev = me.doneProjects[i - 1]
        ...
        const y1 = prev.totalXP / me.totalXP * xpOverTimeGraph.height
        const y2 = curr.totalXP / me.totalXP * xpOverTimeGraph.height
    ...
    for (let i = 0; i < levelChanges.length - 1; i++) {
        ...
        const y1 = (curr.level) / (me.level) * levelOverTimeGraph.height
        const y2 = (next.level) / (me.level) * levelOverTimeGraph.height
...
const drawGraph = (graph) => {
    ...
    description.innerHTML = `<h1>${graph.description}</h1>`
    ...
    document.getElementById('new').innerHTML += container.outerHTML
...
const init = async () => {
    ...
    document.getElementById('login').innerText = `${me.login}`
    document.getElementById('id').innerText = `${me.id}`
    document.getElementById('total-xp').innerText = `${parseInt(me.totalXP.toLocaleString(), 10)-1}`+"KB"
    document.getElementById('level').innerText = `${me.level}`
    
    const xpOverTimeGraph = {
        description: 'TOTAL XP - GRAPH 1',
        width: 1000,
        height: 500,
        topOffset: 500,
        leftOffset: 100,
    ...
    const levelOverTimeGraph = {
        description: "TOTAL LEVEL - GRAPH 2",
        width: 1000,
        height: 500,
        topOffset: 500,
        leftOffset: 100,
   ...
   // document.getElementById('new').innerText = drawGraph(xpOverTimeGraph)
}
